...
- **Button:** `src/components/ui/button.tsx` (shadcn — `Button` + `buttonVariants`, cva). The variants are **site-adapted, not stock** (outline/ghost/link use `bg-surface` + `ring-border`, not shadcn's `border bg-background`). The base includes `no-underline` on purpose — `unlayered.css` underlines every `<a>` in `main`/`.prose` via `a:not(.no-underline)`, so an anchor styled as a button underlines without it. For button styling in `.astro` without a React island, apply `buttonVariants({ variant, size })` directly to an `<a>`/`<button>`.
